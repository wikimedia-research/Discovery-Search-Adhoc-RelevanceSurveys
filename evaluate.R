...
model_accuracy <- cbind(model_index[, 3:5], xgbTree = 0.0, C5.0 = 0.0, nb = 0.0, multinom = 0.0, rf = 0.0, nnet = 0.0, meta = 0.0, dnn = 0.0)
...
for (i in 1:nrow(model_index)) {
  ...
  model_accuracy[i, 4:ncol(model_accuracy)] <- round(accuracies, 4)
  pb$tick()
}; rm(
...
model_accuracy <- tidyr::gather(
  ...
  -c(question, features, discernatron_reliable)
)
readr::write_csv(model_accuracy, file.path("models", "model-accuracy.csv"))
